...
def collision(carts: dict, i: int, ii: int) -> list:
    collisions = []
    for idx, cart in carts.items():
...
result = ()
carts = {idx: cart for idx, cart in enumerate(carts)}
while True:
    new_moves_Q = []
    while len(moves_Q) > 0:
        ...
        collisions = collision(carts, cart[0], cart[1])
        if len(collisions) > 1:
