...
def process_game(input_grid: list, n: int, m: int, elves_AP: int, directions: list) -> (int, bool): 
    id = 0
    players = {}
    players_positions = {}
    moves_Q = []
    ...
    result *= rounds
    return (result, elves_alive == elves_at_start)
...
l, r = 4, 4
while True:
    _, all_elves_alive = process_game(grid, n, m, r, directions)
    if all_elves_alive == True:
        r -= 1
        break
...
while l < r:
    mid = (l + r) >> 1
    _, all_elves_alive = process_game(grid, n, m, mid, directions)
    if all_elves_alive:
        r = mid
    else:
        l = mid + 1

result, _ = process_game(grid, n, m, l, directions)
print(result)