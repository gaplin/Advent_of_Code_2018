...
class Group:
    ...
    def __str__(self) -> str:
        return 'id: {}; type: {}; units: {}; HP: {}; AD: {}; Attack_type: {}; initiative: {}; Weak: {}; immune: {}; EP: {}'\
        .format(self.id, self.unit_type, self.units, self.HP_per_unit, self.AD, self.attack_type, self.initiative, self.weaknesses, self.immunes, self.AD * self.units)
...
def attack(groups: dict, attacks: list) -> int:
    attacks_with_order = []
    dmg_dealt = 0
...
def play(groups: dict) -> None:
    while Two_armys(groups) == True:
        ...
        if attacks == []:
            return
        dmg_dealt = attack(groups, attacks)
        if dmg_dealt == 0:
            return

def simulate_with(groups: dict, boost_value) -> int:
    for group in groups.values():
        if group.unit_type == 0:
            group.AD += boost_value
    play(groups)
    immunes = 0
    for group in groups.values():
        if group.unit_type == 0:
...
while True:
    units_left = simulate_with(deepcopy(groups), r)
    if units_left > 0:
        r -= 1
        break
    l = r
...
while l < r:
    m = (l + r) // 2
    units_left = simulate_with(deepcopy(groups), m)
    if units_left == 0:
        l = m + 1
    else:
        r = m

result = simulate_with(deepcopy(groups), l)
print(result)